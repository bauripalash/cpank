...
pub fn build(b: *std.Build) void {
    const cflags = [_][]const u8{ "-Wall", "-std=c11", "-g3" };

    const sourceToRun = "sample/fib_bn.pank";
    ...
    if (target.isWindows()) {
        ...
        exe.defineCMacro("_ZIG_WIN", "");
        exe.defineCMacro("ZIG_WIN", "");
        lib.defineCMacro("_ZIG_WIN", "");
        lib.defineCMacro("ZIG_WIN", "");
    }
    ...
    const create_exe_shortcut = b.addSystemCommand(&[_][]const u8{ "ln", "-s", "zig-out/bin/pankti", "./" });

    const create_gui_shortcut = b.addSystemCommand(&[_][]const u8{ "ln", "-s", "zig-out/bin/panktiw", "./" });
    ...
    run_code_step.dependOn(cli_build_step);
